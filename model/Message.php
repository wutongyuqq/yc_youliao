...
//OPENTM409023186
//管理员审核提醒
    public static function a_checkmessage($openid,$url,$i_item,$name,$nickname,$createtime,$i_remark = '') {
...
//OPENTM407392801
    ...
    public static function ordermessage($openid,$i_item,$url,$title,$fee,$i_remark = '',$wxapp=0) {
        if($wxapp==1)return;
...
{{remark.DATA}}
	...
	*/
	...
	//客服提醒
    //OPENTM207327169
    public static function kfmsg( $data){
        $item=commonGetData::gettpl('OPENTM207327169');
        if(empty($item['tpl_id']))return;
        $i_id = $item['tpl_id'];
        $title='您有一条未读消息';
        if($data['flag']==1){
...
//{{remark.DATA}}
	//退款申请通知
    public static function applyrefund($openid,$title,$orderid,$fee,$productname,$ogid,$remark) {
        $url2 = Util::createModuleUrl('refund',array('ogid'=>$ogid));
        $item = commonGetData::gettpl('TM00431');
        if(empty($item['tpl_id']))return;
        $i_id = $item['tpl_id'];
        if($item['adv']){
            $remark.= '\\n'.$item['adv'];
...
//退款申请结果通知
    public static function refundresult($openid,$title,$orderid,$fee,$ogid, $remark) {
        $item =  commonGetData::gettpl('OPENTM405486816');
        if(empty($item['tpl_id']))return;
        $i_id = $item['tpl_id'];
        if($item['adv']){
            $remark.= '\\n'.$item['adv'];
...
//{{remark.DATA}}
//待办任务提醒
    public static function userTask($openid,$title,$keyword1,$keyword2,$url, $remark) {
        $item =  commonGetData::gettpl('OPENTM200806215');
        if(empty($item['tpl_id']))return;
        $i_id = $item['tpl_id'];
        if($item['adv']){
            $remark.= '\\n'.$item['adv'];
...
//{{remark.DATA}}
//OPENTM405485000
    public static function amount($openid,$title,$money,$admin_user,$addtime,$id,$paytype,$remark) {
