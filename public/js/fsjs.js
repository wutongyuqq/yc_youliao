		...
		if(type == 'delete'){ //删除订单
			...
			if(confirm('确定要删除此订单吗？')){
				Http('post','html','deleteOrder',postdata,function(data){
					if(data == 1){
						alert('已删除此订单');
						location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=list&do=order&m=yc_youliao';
					}else{
						alert(data);
					}
				},true);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function ajaxUrl(op){
		return window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op='+op+'&do=Ajaxreq&m=yc_youliao';
	}
